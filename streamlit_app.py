...
def load_live_alerts_from_file():
    """Read the shared JSONL log the interception API appends to — works
    when the dashboard and API share a filesystem. Uncached so each rerun
    sees fresh alerts."""
    path = os.path.join(DATA, "flagged_decisions.jsonl")
...
def load_live_alerts_from_api(base_url):
    """Pull the same log over HTTP via GET /v1/alerts — use when the API
    and dashboard do NOT share a filesystem (dashboard on Streamlit Cloud,
    API on Render)."""
    import urllib.request, urllib.error
    try:
        with urllib.request.urlopen(f"{base_url.rstrip('/')}/v1/alerts?limit=200", timeout=4) as resp:
...
def main():
    ...
    # ═════════ ALERT QUEUE ═════════
        ...
        st.markdown("<div class='page-title'>Alert Queue</div>"
                    "<div class='page-sub'>Batch model alerts · plus a live feed from the interception API</div>", unsafe_allow_html=True)
        tab_batch, tab_live = st.tabs(["📊 Batch Model Alerts", "🔴 Live API Feed"])

        with tab_batch:
            if "dis" not in st.session_state: st.session_state.dis = set()
            if "esc" not in st.session_state: st.session_state.esc = set()
            s1,s2 = st.columns([3,1])
            with s1: q = st.text_input("", placeholder="Search account…", label_visibility="collapsed", key="batch_search")
        ...
        with tab_live:
            st.markdown("""<div style='font-size:12.5px;color:#556080;margin-bottom:14px;'>
                Real transactions decided by the running interception API — every BLOCK/STEP_UP/COOLING,
                plus any ALLOW flagged as elevated-but-below-action-threshold. The end-to-end path:
                score → decide → flag → here.</div>""", unsafe_allow_html=True)
            src1, src2 = st.columns([2,3])
            with src1:
                api_url = st.text_input("API base URL", value="http://localhost:8080",
                    help="Where the interception API runs. Used for GET /v1/alerts.")
            with src2:
                mode = st.radio("Source", ["Shared file (same host)", "HTTP — GET /v1/alerts"],
                                horizontal=True)
            if "live_dis" not in st.session_state: st.session_state.live_dis = set()
            if "live_esc" not in st.session_state: st.session_state.live_esc = set()

            if mode.startswith("Shared file"):
                live_alerts, err = load_live_alerts_from_file()
            else:
                live_alerts, err = load_live_alerts_from_api(api_url)

            if err:
                st.info(f"No live alerts yet — {err}\n\nStart the API and POST a few /v1/decide "
                        f"calls to populate this feed.")
            elif not live_alerts:
                st.info("API reachable but no flagged decisions logged yet.")
            else:
                live_alerts = [a for a in live_alerts if a.get("txn_id") not in st.session_state.live_dis]
                st.markdown(f"<div style='font-family:Space Mono;font-size:11px;color:#556080;margin-bottom:10px;'>{len(live_alerts)} live alerts</div>", unsafe_allow_html=True)
                DCOL = {"BLOCK":"#EF4444","STEP_UP":"#F59E0B","COOLING":"#3B82F6","ALLOW":"#556080"}
                for a in live_alerts[:50]:
                    d = a.get("decision","ALLOW"); col = DCOL.get(d, "#556080")
                    risk = a.get("sender_risk_score", 0.0)
                    ts = a.get("logged_at","")[:19].replace("T"," ")
                    st.markdown(f"""<div class='risk-card' style='border-left:3px solid {col};'>
                      <div style='display:flex;align-items:center;gap:12px;'>
                        <div style='font-family:Space Mono;font-size:15px;font-weight:700;color:{col};min-width:78px;'>{d}</div>
                        <div style='flex:1;'>
                          <div style='font-family:Space Mono;font-size:13px;font-weight:700;'>{a.get("txn_id","")} · {a.get("account_id","")}
                          {"&nbsp;<span class='badge badge-amber'>ESCALATED</span>" if a.get("txn_id") in st.session_state.live_esc else ""}</div>
                          <div style='font-size:12px;color:#556080;'>₹{a.get("amount",0):,.0f} · {a.get("txn_flow","")} ·
                          sender risk {risk:.2f} · {ts} UTC</div>
                          <div style='font-size:11.5px;color:#8593AC;margin-top:2px;'>{a.get("review_reason","")}</div>
                        </div>
                      </div></div>""", unsafe_allow_html=True)
                    b1,b2,_ = st.columns([1,1,6])
                    with b1:
                        if st.button("Dismiss", key=f"ld{a.get('txn_id')}"): st.session_state.live_dis.add(a.get("txn_id")); st.rerun()
                    with b2:
                        if st.button("Escalate ↑", key=f"le{a.get('txn_id')}"): st.session_state.live_esc.add(a.get("txn_id")); st.rerun()
